...
if 'question' not in st.session_state:
    ...
    st.session_state['quiz_done'] = False  # Track if the quiz is done or not
...
if submit_answer and not st.session_state['submitted']:
    ...
    if answer == st.session_state['options'][0]:  # Correct answer is the first option
        st.success("✅ Correct!")
    else:
        st.error(f"❌ Incorrect! The correct answer is: {st.session_state['options'][0]}")

# Show the "Next Question" button only after the answer has been submitted
if st.session_state['submitted'] and not st.session_state['quiz_done']:
    next_button = st.button("Next Question")
    
    # Handle next question click
    if next_button:
        # Get the next random question and options
        st.session_state['question'], st.session_state['options'] = random.choice(list(nature_quiz.items()))
        st.session_state['submitted'] = False  # Reset the submission flag for the next question
